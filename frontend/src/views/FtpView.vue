...
const onSelectCard = async (card: typeof cards.value[0]) => {
  ...
  const cacheKey = `${StorageKey.EVENTS}_ftp_${card.type}`;
  const cachedData = storageType.getItem(cacheKey);

  if (cachedData) {
    // Load from cache instantly, no spinner
    records.value = JSON.parse(cachedData);
    return;
  }

  // Only show spinner if fetching from server
  isLoading.value = true;
